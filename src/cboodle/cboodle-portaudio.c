...
/* This source file does not get compiled directly. It must first be
   preprocessed by replacing the token portaudio with one of the
...
extern void initcboodle_portaudio(void);
...
void initcboodle_portaudio(void)
{
  Py_InitModule("cboodle_portaudio", methods);
}
